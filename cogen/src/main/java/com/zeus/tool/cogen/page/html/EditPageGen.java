...
public class EditPageGen implements Cogen{
	...
	public void createClass(String rootPath, DesignEntity designEntity) {
		String filePath = rootPath + "statics/page/" + designEntity.moduleName + "/" + designEntity.instanceName + "-edit.html";
		try {
			File file = new File(filePath);
			if (!file.exists()) {
				String all = FileUtil.fileContent("#instanceName#-edit.html");
				for (DesignField dField : designEntity.dFields) {
					all = GenFieldHelper.gen4EditField(all, dField);
				}
	...
	public static EditPageGen init() {
		return new EditPageGen();
	}
